...
type HeightVoteSet struct {
	...
	peerCatchupRounds map[key.ID][]int32     // keys: peer.ID; values: at most 2 rounds
}
...
func (hvs *HeightVoteSet) Reset(height int64, valSet *types.ValidatorSet) {
	...
	hvs.peerCatchupRounds = make(map[key.ID][]int32)
...
func (hvs *HeightVoteSet) AddVote(vote *types.Vote, peerID key.ID, extEnabled bool) (added bool, err error) {
	hvs.mtx.Lock()
	defer hvs.mtx.Unlock()
