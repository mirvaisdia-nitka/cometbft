...
type BlockPool struct {
	...
	// peers
	peers         map[key.ID]*bpPeer
	bannedPeers   map[key.ID]time.Time
...
func NewBlockPool(start int64, requestsCh chan<- BlockRequest, errorsCh chan<- peerError) *BlockPool {
	bp := &BlockPool{
		peers:       make(map[key.ID]*bpPeer),
		bannedPeers: make(map[key.ID]time.Time),
...
func (pool *BlockPool) RemovePeerAndRedoAllPeerRequests(height int64) key.ID {
	pool.mtx.Lock()
	defer pool.mtx.Unlock()
...
// peer.
func (pool *BlockPool) RedoRequestFrom(height int64, peerID key.ID) {
	pool.mtx.Lock()
	defer pool.mtx.Unlock()
...
func (pool *BlockPool) RedoRequest(height int64) key.ID {
...
func (pool *BlockPool) AddBlock(peerID key.ID, block *types.Block, extCommit *types.ExtendedCommit, blockSize int) error {
...
func (pool *BlockPool) SetPeerRange(peerID key.ID, base int64, height int64) {
	pool.mtx.Lock()
	defer pool.mtx.Unlock()
...
func (pool *BlockPool) RemovePeer(peerID key.ID) {
	pool.mtx.Lock()
	defer pool.mtx.Unlock()
...
func (pool *BlockPool) isPeerBanned(peerID key.ID) bool {
	return cmttime.Since(pool.bannedPeers[peerID]) < time.Second*60
}

func (pool *BlockPool) banPeer(peerID key.ID) {
...
func (pool *BlockPool) pickIncrAvailablePeer(height int64, excludePeerID key.ID) *bpPeer {
	pool.mtx.Lock()
	defer pool.mtx.Unlock()
...
// thread-safe.
func (pool *BlockPool) sendRequest(height int64, peerID key.ID) {
	if !pool.IsRunning() {
		return
	}
...
// thread-safe.
func (pool *BlockPool) sendError(err error, peerID key.ID) {
	if !pool.IsRunning() {
		return
	}
...
func newBPPeer(pool *BlockPool, peerID key.ID, base int64, height int64) *bpPeer {
	peer := &bpPeer{
		pool:       pool,
		id:         peerID,
		base:       base,
...
type bpRequester struct {
	...
	redoCh      chan key.ID // redo may got multiple messages, add peerId to identify repeat
	newHeightCh chan int64

	mtx          cmtsync.Mutex
	peerID       key.ID
	secondPeerID key.ID // alternative peer to request from (if close to pool's height)
	gotBlockFrom key.ID
	block        *types.Block
...
func (bpr *bpRequester) setBlock(block *types.Block, extCommit *types.ExtendedCommit, peerID key.ID) bool {
	bpr.mtx.Lock()
...
func (bpr *bpRequester) requestedFrom() []key.ID {
	bpr.mtx.Lock()
	defer bpr.mtx.Unlock()
	peerIDs := make([]key.ID, 0, 2)
	if bpr.peerID != "" {
...
func (bpr *bpRequester) didRequestFrom(peerID key.ID) bool {
	bpr.mtx.Lock()
	defer bpr.mtx.Unlock()
...
func (bpr *bpRequester) gotBlockFromPeerID() key.ID {
	bpr.mtx.Lock()
	defer bpr.mtx.Unlock()
...
func (bpr *bpRequester) reset(peerID key.ID) (removedBlock bool) {
	bpr.mtx.Lock()
	defer bpr.mtx.Unlock()
...
func (bpr *bpRequester) redo(peerID key.ID) {
	select {
