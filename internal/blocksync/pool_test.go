...
type testPeers map[key.ID]*testPeer
...
func TestBlockPoolRemovePeer(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		peerID := key.ID(strconv.Itoa(i + 1))
		height := int64(i + 1)
	...
	assert.NotPanics(t, func() { pool.RemovePeer(key.ID("Superman")) })

	// remove peer with biggest height
	pool.RemovePeer(key.ID("10"))
...
func TestBlockPoolMaliciousNode(t *testing.T) {
	...
	peers := testPeers{
		key.ID("good"):  &testPeer{key.ID("good"), 1, initialHeight, make(chan inputData), false},
		key.ID("bad"):   &testPeer{key.ID("bad"), 1, initialHeight + MaliciousLie, make(chan inputData), true},
		key.ID("good1"): &testPeer{key.ID("good1"), 1, initialHeight, make(chan inputData), false},
	}
