...
type chunkQueue struct {
	...
	chunkSenders   map[uint32]key.ID          // the peer who sent the given chunk
...
func newChunkQueue(snapshot *snapshot, tempDir string) (*chunkQueue, error) {
	...
	return &chunkQueue{
		...
		chunkSenders:   make(map[uint32]key.ID, snapshot.Chunks),
...
func (q *chunkQueue) DiscardSender(peerID key.ID) error {
	q.Lock()
	defer q.Unlock()
...
func (q *chunkQueue) GetSender(index uint32) key.ID {
	q.Lock()
	defer q.Unlock()
