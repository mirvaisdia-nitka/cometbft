...
func (_m *Mempool) CheckTx(tx types.Tx, sender key.ID) (*abcicli.ReqRes, error) {
	...
	var r1 error
	if rf, ok := ret.Get(0).(func(types.Tx, key.ID) (*abcicli.ReqRes, error)); ok {
		return rf(tx, sender)
	}
	if rf, ok := ret.Get(0).(func(types.Tx, key.ID) *abcicli.ReqRes); ok {
		r0 = rf(tx, sender)
	} else {
		if ret.Get(0) != nil {
	...
	if rf, ok := ret.Get(1).(func(types.Tx, key.ID) error); ok {
		r1 = rf(tx, sender)
	} else {
		r1 = ret.Error(1)
	}
