...
func (mem *CListMempool) addSender(txKey types.TxKey, sender key.ID) error {
	if sender == noSender {
		return nil
	}
...
// Safe for concurrent use by multiple goroutines.
func (mem *CListMempool) CheckTx(tx types.Tx, sender key.ID) (*abcicli.ReqRes, error) {
	mem.updateMtx.RLock()
...
func (mem *CListMempool) handleCheckTxResponse(tx types.Tx, sender key.ID) func(res *abci.Response) error {
...
func (mem *CListMempool) addTx(tx types.Tx, gasWanted int64, sender key.ID, lane LaneID) {
	mem.txsMtx.Lock()
	defer mem.txsMtx.Unlock()
